...
const discoveryPaused = computed(() => {
  const paused=s.configuration?.data?.paused_queues
  return Array.isArray(paused)&&paused.includes('DISCOVER')
})
...
async function refreshData() {
  try {
    const [c,co,ops,caps] = await Promise.all([
...
async function startDiscovery() {
  ...
  try {
    await s.command('start_discovery',{
      ...
      continuous:true,
      cadence_seconds:Math.max(15,Math.min(86400,Number(discoveryCadence.value)||60))
    })
    await refreshData()
    message.value = discoveryProvider.value==='youtube_data'
      ...
      : 'Live discovery patrol started. New internet media will keep flowing through the pipeline automatically.'
  } finally {
      ...
      <section class="start-grid">
        <article class="action-card primary-card live-discovery">
          ...
          <div class="discovery-form">
            ...
            <label>Items / scan<input v-model.number="discoveryBudget" type="number" min="1" max="25"></label>
            <label>Rescan
              <select v-model.number="discoveryCadence">
          ...
          <div class="discovery-actions">
            <button class="primary" :disabled="!systemReady||actionBusy||discoveryPaused" @click="startDiscovery">{{actionBusy?'Working…':'Start Discovery'}}</button>
            <button :disabled="!systemReady||actionBusy" @click="toggleDiscoveryPause">{{discoveryPaused?'Resume Discovery':'Pause Discovery'}}</button>
          </div>
          <small v-if="discoveryProvider==='wikimedia_commons'">Real search + real downloadable source media + real FFmpeg processing. Patrol repeats until paused.</small>
